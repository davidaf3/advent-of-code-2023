...
data WithCost a = WithCost Int Int a

instance Eq (WithCost a) where
  (==) (WithCost c1 _ _) (WithCost c2 _ _) = c1 == c2

instance Ord (WithCost a) where
  compare (WithCost c1 _ _) (WithCost c2 _ _) = compare c1 c2
...
expandCrucible nextSpds grid (WithCost hCost gCost ((x, y), spd, sameDir)) =
  mapMaybe next $ nextSpds sameDir spd
  where
    next spd'@(spdX', spdY') =
      let p' = (x + spdX', y + spdY')
          sameDir' = if spd' == spd then sameDir + 1 else 0
       in getHeat grid p' <&> (\heat -> WithCost hCost (gCost + heat) (p', spd', sameDir'))
...
astar isFinal h expand init =
  ...
  where
    f (WithCost _ cost state) = WithCost (cost + h state) cost state
    go queue visited =
      let (wc@(WithCost _ _ state), queue') = PQ.deleteFindMin queue
              ...
              if isFinal state
                then wc
                  ...
                  let visited' = Set.insert state visited
                      queue'' = foldr (PQ.insert . f) queue' (expand wc)
                   in go queue'' visited'
...
main = do
  ...
  let initStates =
        [ WithCost (h ((0, 0), (1, 0), 0)) 0 ((0, 0), (1, 0), 0),
          WithCost (h ((0, 0), (0, 1), 0)) 0 ((0, 0), (0, 1), 0)
        ]
      rows = V.length grid
      ...
      (WithCost _ cost _) = astar isFinal h expand initStates
      (WithCost _ cost' _) = astar isFinal h expandUltra initStates
  print cost
  print cost'